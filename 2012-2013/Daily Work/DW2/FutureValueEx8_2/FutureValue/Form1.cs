...
namespace FutureValue
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void btnCalculate_Click(object sender, EventArgs e)
        {
            try
            {
                if (IsValidData())
                {
                    ...
                    if (rowcounter < 10)
                    {
                        futureValues[rowcounter, 0] = monthlyInvestment.ToString("c");
                        futureValues[rowcounter, 1] = (interestRateYearly / 100).ToString("p1");
                        futureValues[rowcounter, 2] = years.ToString();
                        futureValues[rowcounter, 3] = futureValue.ToString("c");
                        rowcounter++;
                    }
                    else
                    {
                        MessageBox.Show("Exceeded the max dimension of 10", "Out of range");
                    }
                }
            }
            catch (Exception ex)
        ...
        private void btnExit_Click(object sender, EventArgs e)
        {
            ...
            string msg = "Inv/Mo.\tRate\tYears\tFuture Value\n";
            for (int i = 0; i < rowcounter; i++)
            {
                for (int j = 0; j < futureValues.GetLength(1) - 1; j++)
                {
                    msg += futureValues[i, j] + "\t";
                }
                msg += futureValues[i, 3] + "\n";
            }
